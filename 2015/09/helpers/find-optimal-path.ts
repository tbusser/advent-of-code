import { Graph, NodeList } from './graph.js';
...
function findOptimalPathForLocation(
	...
	const destinations = graph.getDestinationsForLocation(location);
	const numberOfLocations = graph.locations.length;

	function walk(path: string[], destinations: NodeList, distance: number = 0) {
		...
		destinations.keys().forEach(destination => {
			...
			const nextDestinations = new Map(graph.getDestinationsForLocation(destination));
...
export function findOptimalPath(
	...
	// the locations.
	const optimalLengthPerLocation = graph.locations.map(location => {
